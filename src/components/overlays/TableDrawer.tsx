      ...
      <aside className="table-drawer" role="dialog" aria-modal="true" aria-label="Table reference">
        <header>
          <div className="drawer-tabs">
            ...
            <button
              type="button"
              className={tab === 'minutes' ? 'selected' : ''}
  ...
  return (
    <div className="drawer-content rules-content">
      ...
      <p className="rules-lede">
        Every player owns one country. You win together only when every country signs before Round 6
        ends.
      </p>
      <ol className="round-sequence">
        <li>
          ...
          <div>
            ...
            <p>Choose one of three policies. Build resources, lower pressure, or cooperate directly.</p>
          </div>
        </li>
        <li>
          <span>II</span>
          <div>
            <strong>Crisis council</strong>
            <p>
              Commit what your country can spare. Contributions are spent even if the requirement is
              missed.
            </p>
          </div>
        </li>
        <li>
          ...
          <div>
            ...
            <p>Make one move: sign, exchange, build a backchannel, or pass.</p>
          </div>
        </li>
      </ol>
      ...
      <div className="rule-grid">
        <article>
          ...
          <p>Meet your country’s private political requirement.</p>
        </article>
        <article>
          <strong>Red line</strong>
          <p>Restore any breached red line before signing.</p>
        </article>
        <article>
          <strong>Peace 6+</strong>
          <p>Build enough shared momentum for a credible accord.</p>
        </article>
        <article>
          <strong>Trust 2+</strong>
          <p>Your average relationship with the table must reach 2.</p>
        </article>
      </div>
      <h3>Trust is a resource</h3>
      <p>
        Countries build Trust by carrying a fair share of crises, completing exchanges, using
        diplomatic policies, and opening backchannels. If one country contributes nothing while
        another carries the crisis, their relationship weakens.
      </p>
      <h3>Shared defeat</h3>
      ...
      <div className="resource-reference">
        <h3>Resources</h3>
        {(Object.keys(RESOURCE_META) as Resource[]).map((resource) => (
          <span key={resource}>
            <i className={`resource-dot resource-${resource}`} />
          ...
          <article key={entry.id}>
            ...
            <div>
              <small>
                ...
                {entry.country ? ` · ${COUNTRY_DEFINITIONS[entry.country].name}` : ''}
              </small>
              <p>{entry.message}</p>
            </div>
          </article>
